/*
    ABOUT: Table creation using sqlx migrations
    ...
    that has a white-haired woman with blue eyes, wearing a chinese clothes.

    This image would have tags like:
    `white-haired_female`, `blue_eyes`, `chinese_clothes`

    ___________________________________________________________________________
   
    For this lesson the focus will be on creating a table called `tags`,
    that will store the name of each tag and a unique id for that tag
    that will be generated by Postgres.

    I'm leaving this at two fields to keep this simple.

    ___________________________________________________________________________
    
    First install the sqlx-cli globally:
    ...
    Creating migrations/20260709200928_create_tags_table.sql

    ___________________________________________________________________________
    
    This will create a `migrations` directory at the root of your project

    With a file called 20260709200928_create_tags_table.sql

    The number is a timestamp of when the file was created.
